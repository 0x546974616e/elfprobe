...
/// ```
///
/// And a struct or a tuple:
///
/// ```txt
/// struct #IDENTIFIER #GENERICS where #WHERE {...};
/// struct #IDENTIFIER #GENERICS (...) where #WHERE;
/// ```
///
...
/// ```
///
pub(crate) fn derive_empty_trait(stream: TokenStream, r#trait: &str) -> TokenStream {
  ...
  // B. "impl" keyword
  derive.extend([TokenTree::from(Ident::new("impl", Span::call_site()))]);
  ...
  // E. "for" keyword
  derive.extend([TokenTree::from(Ident::new("for", Span::call_site()))]);
